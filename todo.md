...
- [ ] 接入已確認的付款服務，建立付款成功／失敗／取消狀態處理
- [ ] 以付款服務 webhook 或等效事件同步訂單與庫存，避免重複扣庫存
- [ ] 補上商品銷售所需的配送、退換貨、付款說明及私隱合規內容
- [ ] 完成支付流程、庫存邊界、後台手機版、TypeScript／Vitest 驗證並保存檢查點

## 2026-08-27 Stripe 帳戶連接排查
- [ ] 確認 Stripe 帳戶已登入、測試／正式模式與專案付款設定的連接狀態
- [ ] 確認測試 API Key 或專案 Stripe sandbox 認領流程可用，且不在對話中暴露秘密金鑰
- [ ] 完成測試付款、取消付款、webhook 及庫存同步驗證後更新電商 TODO
...
- [ ] 驗證管理員可新增商品與上傳圖片，並確認一般帳戶仍不能操作後台
