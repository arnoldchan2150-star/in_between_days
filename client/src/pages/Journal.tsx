    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="pt-32 pb-12 bg-background border-b border-border">
        <div className="container">
          <p className="text-xs text-muted-foreground tracking-[0.2em] uppercase mb-2">Journal</p>
          <h1 className="font-serif text-3xl font-light">旅行遊記</h1>
        </div>
      </section>
              ...
              {posts.map((post, i) => (
                <Link key={post.id} href={`/destinations/${encodeURIComponent(post.slug)}`}>
                  <article className="group cursor-pointer">
                    ...
                    <h2 className="font-serif text-lg font-light mb-2 group-hover:text-muted-foreground transition-colors">
                      {post.title}
                    </h2>
                    {post.excerpt && (
